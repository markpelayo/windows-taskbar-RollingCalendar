...
It is deliberately lightweight and easy on RAM. There is one window, one one-second timer and no other scheduled work; nothing is allocated on the repaint path; events are held in memory and never written to disk; and there is no runtime to install, because the binary is statically linked and pulls in no third-party libraries. [Resource usage](#resource-usage) sets out what that does and does not amount to — nobody has profiled it.

This is version 1.1.0. It has been run, and works, on Windows 11 24H2 on one machine, at two resolutions. Please read [What has and has not been tested](#what-has-and-has-not-been-tested) before you decide how much to trust it.
...
**A failed fetch is retried after fifteen seconds**, doubling on each further failure up to the ordinary five-minute interval. A failure at launch is almost always a network stack that has not finished coming up at log on, and it clears in seconds; waiting out the full five minutes for that meant five minutes of "Calendar unreachable" on a machine that was in fact online. The doubling is there so that a feed which is genuinely gone is not hammered.

**Both fetch paths re-lay-out immediately** rather than waiting for the next tick. The labels have just changed — from nothing, or from an error, to a full day, or the other way — and until the widget is resized to match, the window is the wrong width for what is being drawn into it.

**The timeline is given its space before the gutters get theirs.** The strip's width is finite and the labels will happily consume all of it. Reserving the capsule area first means the strip cannot collapse into two overlapping labels with no blocks between them, which is the one failure mode that would leave the widget saying nothing at all.
...
| Item | What it does |
|---|---|
| `Rolling Calendar 1.1.0 · by markpelayo` | Opens the project page in your browser. |
| `Week 36 · Tuesday · September 1, 2026 · <source>` | Caption, not clickable. ISO 8601 week number, the date in the display zone, and the name of the calendar being read. Gains `· ! Simulated !`, in red bold, when Debug Time is on — the same words the strip itself shows, so the two cannot be read as different states. |
...
| *the day's blocks* | Up to 60 rows: a marker, a bold title and a faint highlight behind the row for the block you are in, then start and end times, duration, title, a category swatch and name, and an `(N) Overlapped` badge where one applies. Date separators appear where the day changes. Beyond 60 rows a final row reads `… and N more`. Not clickable — this is a listing, not a set of commands. |
| — | |
...
| `Saved Calendars ▸` | Replaces the row above once anything is saved. Ticked in the top-level menu when a saved calendar rather than the demo is the source, so which of the two rows is live can be read without opening the submenu. One row per calendar; clicking one activates it, and the tick marks the one being read. Each has an `Edit "name" ▸` submenu with **Rename** and **Remove**. Ends with `Add Calendar…`. |
...
| — | |
| `Time Range: ±1 hour ▸` | The visible span: ±5, ±10, ±15, ±30 minutes, ±1 hour, ±2 hours, with `(default)` on the ±1 hour row. |
| `Timeline Width: 250 pt ▸` | 100 to 450 points in steps of 50, `(default)` on 250, and the smallest and largest labelled as such. The note underneath tells you how wide a fifteen-minute block currently is. |
| `Labels ▸` | Four toggles: left gutter block name and time left, right gutter block name and how long it runs. Overlap warnings always show and cannot be turned off. This is the one geometry row with no value in its title, because four independent toggles have no single value to report. |
| `Label Length: 360 pt ▸` | 100 to 480 points, each labelled with roughly how many characters that is — measured at runtime in the actual font, not hard-coded — and `(default)` on 360. Long names are shortened with an ellipsis; the countdown and the overlap badge are never cut. |
| `Text Size: <state> ▸` | **Default (matches the taskbar)**, which is the shell's own menu-font size and what everything else in the bar draws with, then 11, 13 and 15 pt, then one custom size from 6 to 48 pt, edited in place rather than added to a list. The row reads **Add Custom…** until a size has been typed and **Custom: N pt…** thereafter. One custom rather than a growing removable list, because only one size is ever in force and a list you can add to but not take from fills up permanently. The typed size is hidden from the list when it coincides with a preset, so the radio group never carries two marks. |
| `Timeline Height: <state> ▸` | The height of the coloured band, in logical pixels: 16, 20, **Default (24 px)**, 28, 32, plus one custom from 8 to 64 px on the same terms as Text Size. The default keeps its place in the ascending order rather than being lifted to the top, because the list is a scale and a scale with a rung out of sequence is harder to read than one with a word on a rung. The band is trimmed to fit when the taskbar is shorter than the chosen height. |
| `Reset Strip Settings` | Resets the strip's geometry and nothing else: time range, timeline width, the four label toggles, label length, text size and its custom, and timeline height and its custom. Greyed out when they are all already at their defaults. Renamed from "Restore Strip Settings", because two rows in one menu both beginning "Restore" read as the same action twice and the heavier of the two is further down. |
| — | |
...
| — | |
| `Run at Startup: <state> ▸` | **Off**, **On (no delay)**, then a delay of 5, 10, 15, 20, 30 or 60 seconds. Exactly one row carries the tick at any time: while startup is off every delay row reads unticked, which is the honest picture, since none of them is in force. The tick reflects the registered scheduled task rather than the stored flag, so deleting the task in Task Scheduler is not reported back as still on. See [Run at startup](#run-at-startup). |
| — | |
...
The five geometry submenus carrying a value in their title — Time Range, Timeline Width, Label Length, Text Size and Timeline Height — do so for the same reason: the setting in force should be readable from the parent row rather than requiring the submenu to be opened and its ticks scanned.
...
| `[strip]` | `titleFontSize` | `0` | Gutter text size in points. `0` means the shell's own menu-font size. Text Size. |
| `[strip]` | `customFontSize` | `0` | The one text size you have typed, kept so it can be chosen again or edited without retyping. `0` means none has been typed. |
| `[strip]` | `blockHeight` | `0` | Height of the capsule band in logical pixels. `0` means the built-in 24, chosen to match a taskbar icon so the strip sits alongside them rather than filling the bar. Timeline Height. |
| `[strip]` | `customBlockHeight` | `0` | The one height you have typed, on the same terms as `customFontSize`. |
...
Nothing the app could ask about its own window would have found that. Every measurement it could take of itself said the strip was correct — visible, sized, positioned, at the front of the taskbar's children — and it still could not be seen. It took an inspection of the shell's own child z-order, with our window identified in it, to establish that the sibling order was not the problem and that the compositor was.
...
- **It looks for another application's widget before parking.** Two apps using this technique would otherwise both claim the same spot and one would silently cover the other, so the automatic placement finds the rightmost gap before the tray that is not already occupied. When there is no clear gap it parks beside the tray and overlaps deliberately — that is at least where you would look for it — which from the outside is indistinguishable from a placement bug, so **Move widget…** exists to put it somewhere with room.
...
The app is meant to be lightweight and easy on RAM, and to be indistinguishable from idle. The design choices that make it so are worth listing plainly, because they are the reason it can afford to redraw once a second.

**Nothing here has been profiled.** No memory figure and no CPU figure has been measured, so none is quoted. What follows is what the design guarantees, which is a different and weaker claim than a benchmark — but it is the one that can be made honestly.

- **One window and one timer.** A single one-second tick redraws the strip, re-measures the widget, checks the alerts and checks the chime. Nothing else is scheduled, and there is no second window, no hidden message loop and no background poll.
- **Nothing is allocated on the repaint path.** Every GDI handle in the codebase — brush, pen, font, bitmap, region — is owned by an RAII wrapper, and the brushes, pens, fonts and back buffer used by the strip are cached and rebuilt only when a colour, the DPI or the size changes. This is a correctness requirement as much as an economy: a Windows process is capped at 10,000 GDI handles, and leaking one brush per redraw at one redraw a second would exhaust that in under three hours and kill the app.
- **A single worker thread, and only while a fetch is in flight.** It exists for the duration of the download and then exits. Between fetches the process has nothing running but its message loop.
- **Events are held in memory only** and never written to disk. They exist while the app runs and are gone when it exits. Recurrence is expanded for four days, not for the feed's whole history, so the working set is a day either side of today rather than a calendar.
- **No third-party libraries, and nothing to install.** The binary is statically linked and needs no Visual C++ redistributable, no runtime and no framework. Nothing is loaded at startup that the app did not compile itself.
...
- **The feed is fetched on that same tick**, every five minutes, rather than on a timer of its own.
...
- **The taskbar's theme is read from the registry at most every couple of seconds** and invalidated outright on the theme-change message, rather than being read afresh by each of the several places per tick that need it.
- **The tray tooltip is composed at most every five seconds.** It used to be composed every second and then thrown away unchanged, which is work done for nothing on all but one tick in five.
...
The code has had a correctness, leak and performance review, but it has never been run under a profiler or a leak detector, so no memory or CPU claim in this README rests on measurement.

If you run it in any of the untested configurations, a report either way is the most useful thing you could send. Your Windows build number, your display scaling and which `hostOverride` value produced a visible strip are what make such a report actionable.
...
The escape hatch is `hostOverride`, under `[hidden]` in `%APPDATA%\RollingCalendar\settings.ini`. Edit it with the app closed and start it again. `0` is automatic, `1` a plain child of the taskbar, `2` a layered child, `3` a floating window over the taskbar.

1. Set `hostOverride=3`. That forces the floating window, which is the mode with the fewest ways to fail. If the strip appears there, the embedding is the problem rather than the drawing.
2. Set `hostOverride=1` for a plain, unlayered child. On a Windows 11 taskbar this will most likely be invisible, for the reasons described above, but it is the right thing to try on a machine where layering itself misbehaves.
3. Report which of the settings worked, along with your Windows build number. A machine where this fails is more useful to the project than a machine where it works.
...
**The strip says `! Simulated !` in red bold.** Debug Time is on and the clock you are looking at is not the real one. **Reset to Current Time** in the menu clears it.
...
- **Nothing is logged to disk.** The app writes no log file of any kind, so there is no second place your day can end up.
