...
[![platform](https://img.shields.io/badge/platform-Windows%2010%20%7C%2011%20x64-0078D4)](https://github.com/markpelayo/windows-taskbar-RollingCalendar)
[![licence](https://img.shields.io/badge/licence-MIT-blue)](LICENSE)

A small strip that lives in the Windows taskbar and draws today's calendar as a horizontal timeline, scrolling right to left past a fixed red line marking now. It is a C++/Win32 port of [macos-menubar-RollingCalendar](https://github.com/markpelayo/macos-menubar-RollingCalendar), which does the same thing in the macOS menu bar. Events come from a public iCalendar feed you paste in, or from a built-in demo day. One executable, no installer, no dependencies beyond the Windows SDK.

This is version 1.0.0. It is a fresh port. Please read [Limitations](#limitations) before you decide to trust it with anything.

---

## The UI

There is no screenshot yet. The app has been written but not photographed on a
real machine, and a mocked-up image of software nobody has run is worse than no
image at all. So, in words.

The widget sits in the taskbar immediately to the left of the clock and the notification icons. It is one row tall — as tall as the taskbar allows, minus a couple of pixels of clearance — and somewhere between 250 and 700 pixels wide depending on how long your event names are.

Reading left to right:

- **The left gutter.** Right-aligned text naming the block you are in and how long is left of it: `Deep Work (5m)`. When there are two minutes or less remaining, the whole label turns red and bold.
- **The strip.** A row of coloured capsules. No tick marks, no gridlines, no times written on the blocks. A red vertical line sits at the exact centre.
- **The right gutter.** Left-aligned text naming what is next and how long that runs: `(16h) Out of office`.

Either gutter can carry a small red dot with a number next to it. That is the overlap badge, and its position matters — see [How it works](#how-it-works).

Left-click the strip and a menu drops down listing the day's blocks with their times, durations, categories and overlaps, plus every setting the app has. Right-click does the same thing. There is also an icon in the notification area, next to the clock, which opens the same menu; it exists so that there is always a documented, guaranteed way to reach the app even if the strip fails to appear.
...
Rolling Calendar removes the question. You glance at the taskbar and you can see it: how much of the current block has already gone grey behind the now line, how much colour is still ahead of it, whether the next thing starts in eight minutes or four hours, and whether anything is about to collide.
...
**Overlaps are flagged, never stacked.** The strip is always one row. Where two blocks collide, the longer one is drawn first and the shorter one on top of it, so the short thing stays visible. The fact that there is a collision at all is reported by a badge in the gutter: a red dot followed by a count.
...
Every push to `main` produces a build artifact, and every tagged release has `RollingCalendar-windows-x64.zip` attached. Inside is a single `RollingCalendar.exe`. There is nothing to install: unzip it somewhere permanent and run it.

Requires Windows 10 version 1607 or later, or Windows 11, on x64. The binary is statically linked, so it needs no Visual C++ redistributable.

### Build

The only prerequisite is MSVC and the Windows SDK. There is no vcpkg manifest, no NuGet restore, no submodules and no third-party libraries. Visual Studio 2022 Community or the standalone Build Tools both provide what is needed.
...
The result is `build\RollingCalendar.exe`.
...
1. **Run `RollingCalendar.exe`.** A strip appears in the taskbar, to the left of the clock. If you cannot see it, look for the new icon in the notification area — clicking that opens the same menu.
2. **Look at the demo day.** On first launch the app shows a built-in demo calendar: a realistic time-blocked day with two deliberate collisions in the afternoon, so you can see what the overlap badges look like before you have any overlaps of your own. Nothing is being fetched from anywhere.
3. **Right-click the strip** to open the menu.
...
| `Week 35 · Monday · August 24, 2026 · <source>` | Caption, not clickable. ISO 8601 week number, the date in the display zone, and the name of the calendar being read. Gains `· Simulated` in red when Debug Time is on. |
...
| *the day's blocks* | Up to 60 rows: a marker and bold title for the block you are in, then start and end times, duration, title, a 10-pixel category swatch and name, and an overlap badge where one applies. Date separators appear where the day changes. Beyond 60 rows a final row reads `… and N more`. Not clickable — this is a listing, not a set of commands. |
...
| `Saved Calendars ▸` | One row per saved calendar; clicking one activates it, and the tick marks the one being read. Each has an `Edit "name" ▸` submenu with **Rename** and **Remove**. Ends with `Add Calendar…`. |
...
| `Keyword Colors ▸` | Shows how many rules are loaded and where they came from, one dim row per category with its swatch and rule count, plus `Uncategorized`. Then **Import CSV…**, **Use Sample Colors**, **Save Sample CSV…** and, when rules exist, **Clear Keyword Colors**. |
| `Restore Strip Settings` | Resets the seven appearance settings above. Greyed out when they are all already at their defaults. |
...
| `Restore Defaults…` | Deletes every setting. The confirmation lists exactly what will go and **Cancel** is the default button, because Return should not be the fast path to a wipe. Greyed out when everything is already at its default. |
...
It is read once at launch, held in memory and written through on change, so a redraw never touches the disk. You can edit it by hand with the app closed.
...
Most settings have menu items. These do not — they exist because they are worth having but not worth a menu row:

| Key | Default | What it is |
|---|---|---|
| `nowLineWidth` | `4` | Thickness of the red now line, in points. |
| `urgentSeconds` | `120` | How long before a block ends the left gutter turns red and bold. |
| `unmatchedColor` | `#8E8E93` | Colour for events that match no keyword rule. |
| `solidBlocks` | `true` | `false` draws translucent tinted blocks instead of solid ones. |
| `blockGap` | `1` | Cosmetic gap between adjacent blocks, in points. Never alters the time span a block represents. |
| `blockCornerRadius` | `0` | `0` means a full capsule, that is, radius equal to half the height. |
| `titleFontSize` | system menu size | Text size in the gutters. |
| `dayAnchorKeyword` | `sleep` | What opens and closes the dropdown's day. |
| `widgetOffsetFromRight` | `-1` | Where the strip sits, as a distance from the taskbar's right edge. `-1` means automatic placement left of the notification area. |

All of these are read as "zero or missing means the default", so a value must be greater than zero to take effect. `blockCornerRadius` is the exception, since zero is a meaningful choice there.
...
Emptying the list is the same thing as switching it off. When it is off, every row reads as unticked, so a click means "turn this on" rather than "delete this"; the `Remove` rows are the permanent delete.
...
This is the one genuinely unsupported thing the app does, so it deserves a plain explanation rather than a footnote.
...
What is left, and what the app does, is to create an ordinary child window and re-parent it into the taskbar's own window — the one whose class name is `Shell_TrayWnd` — using `SetParent`. The child then moves, hides and auto-hides along with the taskbar, because as far as the shell is concerned it is part of it.

This is the same technique as the author's [windows-taskbar-pinger](https://github.com/markpelayo/windows-taskbar-pinger), which established that it works and what it costs. This is the second application to use it in the same hands, which is the only reason it was worth attempting something this large on top of it.

**It is written entirely from Microsoft's public documentation for `FindWindow`, `SetParent` and `SetWindowPos`.** Every one of those functions is documented and supported. What is not documented, and what Microsoft has never committed to, is that the taskbar will tolerate a foreign child window living inside it. That is emergent shell behaviour. It happens to work; nobody has promised it will keep working.

The consequences, stated plainly:

- **A future Windows update could break it.** Not "might behave oddly" — could stop it working entirely. There is no contract here to appeal to.
...
- **Every failure path falls back to floating.** If the taskbar window cannot be found, if `SetParent` fails, if the machine is locked down or the shell has been replaced, the app puts up an always-on-top window positioned over the taskbar instead. Less integrated, and it will not hide when the taskbar hides, but it is never invisible.
- **The notification-area icon is the escape hatch.** The strip lives among the taskbar's own children and could in principle end up behind a crowded row of pinned applications. An icon in the notification area is a documented, guaranteed way to reach the menu when that happens.
...
| | macOS | Windows |
|---|---|---|
| The strip | `NSStatusItem` with a custom view of any width | A child window re-parented into `Shell_TrayWnd`, with a floating fallback. There is no supported equivalent; see above. |
...
| Overlap badge | A red circle emoji, `🔴(2)` | A drawn red dot followed by the count. Emoji rendering in an `HMENU` and in taskbar text is inconsistent across Windows versions and fonts; a filled ellipse is not. |
...
8. **A failed refresh keeps the last good day for thirty minutes**, then clears it. Until then the strip shows the error with yesterday's plan underneath, because showing a stale day beats showing nothing. After thirty minutes it really is unknown.
...
13. **The taskbar embedding is undocumented shell behaviour.** Microsoft has never committed to it. See the section above for what that means in practice.
14. **MSVC only.** MinGW may work but the manifest and resource handling differ, and it is not tested.
15. **x64 only.** No ARM64 build is published.

And the honest part:

16. **This is version 1.0.0 and it has not yet been run on a real machine by the author.** It compiles cleanly as one static binary and CI confirms it launches and stays up, which is a much weaker claim than it sounds. Specifically **untested**: Windows 10 of any build, fractional display scaling, taskbars docked to the left or right edge, and multi-monitor setups. Those are precisely the configurations where a widget that positions itself by measuring the taskbar is most likely to be wrong, and reports from them are the most useful thing anyone could send. Please do not describe this software to anyone else as battle-tested.

---

## Troubleshooting

**Nothing appeared.** Look near the clock rather than in the middle of the taskbar — the strip anchors to the notification area, not to the app buttons. If it genuinely is not there, look for the Rolling Calendar icon in the notification area, which is always added regardless of whether the strip embedded successfully. Clicking it opens the same menu, and its tooltip carries the same text the gutters do. If the icon is there and the strip is not, the embedding failed and the app is running in floating mode, or the floating window has ended up somewhere unexpected. **Reset widget position** is the first thing to try.
...
**The strip says "No calendar yet — click to set one up".** No calendar is configured and demo mode is off. Right-click, **Add Calendar**, paste a link.
